...
public class WebCrawler12 {
	...
	public void crawlMap(String location) {
		...
		for (WebElement e : elements) {
			...
			// 수집된 이미지 URL 출력
			List<WebElement> imageElements = driver.findElements(By.cssSelector(".K0PDV._div"));
			for (int i = 0; i < imageElements.size(); i++) {
				String styleAttribute = imageElements.get(i).getAttribute("style");
				String imageUrl = styleAttribute.split("url")[1].split("\"")[1];
				System.out.println("이미지 URL " + (i + 1) + ": " + imageUrl);
			}

			// 이미지 정보 출력
			System.out.println("이미지 정보: " + e.findElement(By.cssSelector(".place_thumb")).getAttribute("href"));

			driver.switchTo().parentFrame();
			driver.switchTo().frame(driver.findElement(By.id("entryIframe")));

			// 나머지 데이터 수집 코드는 동일하게 유지됩니다.
